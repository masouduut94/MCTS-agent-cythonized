...
class rave_mast_mctsagent(rave_mctsagent):
  ...
  def __init__(self, state=gamestate(8)):
    super().__init__(state)
    self.EPSILON = 0.4
    self.GIBBS_CONSTANT = 10
    cdef list all_moves = self.rootstate.moves()
  ...
  def roll_out(self, state):
    ...
    black_q = list(map(lambda x, y: np.exp((x/y) / self.GIBBS_CONSTANT) if y != 0 else 0, black_table[1, :], black_table[2, :]))
    black_table = np.vstack((black_table, black_q))
    # white_table creation
    white_q = list(map(lambda x, y: np.exp((x/y) / self.GIBBS_CONSTANT) if y != 0 else 0, white_table[1, :], white_table[2, :]))
    ...
    for item in moves_played:
      if state.board[item] == self.rootstate.PLAYERS["black"]:
        self.black_moves.append(item)
      elif state.board[item] == self.rootstate.PLAYERS['white']:
        self.white_moves.append(item)

    return state.winner(), []

  def backup(self, node, turn, outcome, pl_length=None):
    ...
    while node!= None:
      if turn == self.rootstate.PLAYERS["white"]:
        for point in self.white_moves:
          if point in node.children:
            node.children[point].Q_RAVE += reward
            node.children[point].N_RAVE += 1
      else:
        for point in self.black_moves:
          if point in node.children:
            node.children[point].Q_RAVE += reward
            node.children[point].N_RAVE += 1
      ...
      node.Q += reward
      if turn == self.rootstate.PLAYERS["black"]:
        turn = self.rootstate.PLAYERS["white"]
      else:
        turn = self.rootstate.PLAYERS["black"]
      reward = -reward
      node = node.parent
    
    if outcome == self.rootstate.PLAYERS['black']:
      for i in range(self.scoreboard.shape[1]):
        for move in self.black_moves:
          ...
          if self.scoreboard[0,i] == move and self.scoreboard[1,i] == self.rootstate.PLAYERS['black']:
            # for winner update the playouts and the wins
            self.scoreboard[3, i] += 1
            self.scoreboard[2, i] += 1

        for move in self.white_moves:
          if self.scoreboard[0,i] == move and self.scoreboard[1,i] == self.rootstate.PLAYERS['white']:
